...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Agregamos nav_mensajes como destino de nivel superior
        mAppBarConfiguration = new AppBarConfiguration.Builder(
                R.id.nav_inicio, R.id.nav_iniciar_sesion, R.id.nav_devocional, R.id.nav_mensajes)
                .setOpenableLayout(drawer)
                .build();
    ...
    public boolean onSupportNavigateUp() {
        ...
        navigationView.setNavigationItemSelectedListener(item -> {
            ...
            // Manejo automático de navegación para el resto de los ítems (incluido nav_mensajes)
            boolean handled = NavigationUI.onNavDestinationSelected(item, navController);
            if (handled) {
                DrawerLayout drawer = findViewById(R.id.drawer_layout);
                drawer.closeDrawers();
    ...
    private void cerrarSesion() {
        ...
        invalidateOptionsMenu(); // Actualiza la visibilidad del menú lateral
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        MenuItem itemSalir = drawerMenu.findItem(R.id.nav_salir);
        MenuItem itemLogin = drawerMenu.findItem(R.id.nav_iniciar_sesion);
        MenuItem itemMensajes = drawerMenu.findItem(R.id.nav_mensajes); // Nuevo ítem para administrar

        if (token != null) {
            // Usuario autenticado: mostramos gestión y salida
            if (itemSalir != null) itemSalir.setVisible(true);
            if (itemLogin != null) itemLogin.setVisible(false);
            if (itemMensajes != null) itemMensajes.setVisible(true);
        } else {
            // Usuario anónimo: ocultamos gestión
            if (itemSalir != null) itemSalir.setVisible(false);
            if (itemLogin != null) itemLogin.setVisible(true);
            if (itemMensajes != null) itemMensajes.setVisible(false);
        }
